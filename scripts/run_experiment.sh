...
RUNS="${RUNS:-1}"
# Which corpus stage 1 draws from; see the stage 1 comment below for the choice.
ARM="${ARM:-saecorpus}"
# Arm is in the filename so two corpora cannot silently overwrite each other --
# the parquet is skipped when it already exists, and an arm switch would
# otherwise reuse the previous arm's activations without saying so.
PARQUET="${PARQUET:-acts_${ARM}${N_DOCS}_L32.parquet}"
OUT="${OUT:-results}"
...
# ---- 1. corpus ---------------------------------------------------------------
# Chat data, because on FineWeb this same pipeline measured a 7-point effect
# where chat gave 25 -- FineWeb is out of distribution for the SAE.
#
# ARM=saecorpus takes conversations from the `tokens` array inside Gemma Scope's
# examples.safetensors: 236,783 finished Gemma chats, already downloaded by the
# setup step and already read by stage 4 to label latents. Nothing is generated,
# so this stage costs one forward pass per conversation instead of ~400 decode
# steps -- minutes rather than hours -- and the text is byte-identical on every
# re-run.
#
# ARM=rollout is the older path: reproduce Gemma Scope's recipe ourselves by
# sampling oasst1 + LMSYS prompts and generating responses with Gemma. Slower
# and not bit-reproducible across model versions, kept so the two corpora can be
# compared.
echo "=== 1/5  corpus + activations (arm=$ARM) ==="
if [ -f "$PARQUET" ]; then
  echo "    $PARQUET exists, skipping (delete it to rebuild)"
else
  python "$SRC/extract_activations.py" --arm "$ARM" \
      --n-docs "$N_DOCS" --positions-per-doc 1 --seed 42 --out "$PARQUET"
fi
# Cheap to rebuild under saecorpus, but still backed up: it is the join key for
# every downstream artefact, and under --arm rollout it is hours of generation.
