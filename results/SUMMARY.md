...
Does the AV's final paragraph -- shown elsewhere to carry most of the round trip's FVE -- also carry more of what is genuinely IN the activation, or only more of what the AR can rebuild? Rows 1-2 need no judge or labels (the SAE checking itself); rows 3-4 are where the judge and labels enter.
...
| **3. grounded, of labeled** F-orig latents | 43.0% | 19.6% (46% of full) | 41.6% (97% of full) |
| **4. grounded, of ALL** F-orig latents (conservative) | 19.5% | 8.9% (46% of full) | 18.9% (97% of full) |

- row 3 denominator: 1,834 labeled F-orig latents. Row 4 denominator: 4,036 -- every F-orig latent, whether or not it could be labeled and checked. Row 4 < row 3 by construction: same numerator, larger denominator.
- rows 1-2 and rows 3-4 come from independent measurements: raw SAE-latent overlap with no text or judge involved (1-2), versus an LLM judge reading text against labelled latents (3-4). If `final_only` tracks `full` on both, two independent signals agree on where the content is.
